...
namespace bazy1 {
	public class PrescriptionGenerator {
		public void generate(Prescription prescription) {
				...
				container.Page(page =>
				{
					...
					page.Content().Table(table =>
					{
						...
						table.Cell().Row(2).ColumnSpan(2).BorderLeft(1F).Text("Pacjent\n"+
							prescription.Patient.Name +" "+prescription.Patient.Surname+"\n"+prescription.Patient.Addresses.First().City+" "+prescription.Patient.Addresses.First().Street+" "+
							prescription.Patient.Addresses.First().BuildingNumber ).AlignLeft();
						...
						for(int i = 0; i < 12; i++)
						{
							...
							table.Cell().Row((uint)i + 5).ColumnSpan(2).BorderLeft(1F).BorderRight(1F).Text(
