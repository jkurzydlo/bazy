...
namespace bazy1.ViewModels.Admin.Pages {
	public class AddPatientAdminControl : ViewModelBase{
		...
		public AddPatientAdminControl(AdminPatientListViewModel parentViewModel) {
			...
			this.doctor = doctor;
			ShowPatientListCommand = new BasicCommand(obj => parentViewModel.ShowPatientListCommand.Execute(null));
			AddNewPatientCommand = new BasicCommand((object obj) =>
			{
				...
				if (ErrorCollection.Count == 0)
				{
					...
					if (validate(Email))
					{
						...
						if (MailAddress.TryCreate(Email, out tempMail)) Email = Email;
					}
					/*
					patient.Name = Name;
					...
					DbContext.Database.ExecuteSqlRaw($"insert into patient_address values((select id from patient where pesel= '{Pesel}'),last_insert_id())");

					if (SecondAddressVisible == Visibility.Visible)
					{
						DbContext.Database.ExecuteSqlRaw($"insert into address(street,city,postalCode,buildingnumber,type) values('{Street2}','{City2}','{PostalCode2}','{BuildingNumber2}','Zameldowania')");
						DbContext.Database.ExecuteSqlRaw($"insert into patient_address values((select id from patient where pesel= '{Pesel}'),last_insert_id())");
					}
